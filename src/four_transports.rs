#![forbid(unsafe_code)]
//! The four avenues this web server can use to reach `hhm-api`.
//!
...
//! to hacker-house-medellin — the environment prefix, the service slug that names its NATS
//! subjects, and the api route that accepts an envelope.
//!
//! | # | Mode | What happens |
//! |---|------|--------------|
//! | 1 | `direct_read` | this server queries Postgres itself through `hhm-lib-core`, read-only |
//! | 2 | `http` | stateless request to the `hhm-api` cluster |
//! | 3 | `tcp` | framed, held-open connection to the `hhm-api` cluster |
...
//! `hhm-lib-core`'s read context, and hand it to [`gateway_from_env`]. See
...
///
/// `HHM_API_URL`, `HHM_NATS_URL`, and the rest; see the
/// table in `ores-transport`'s README.
pub const ENV_PREFIX: &str = "HHM";
...
pub const SERVICE_SLUG: &str = "hhm";
